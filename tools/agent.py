...
async def post(text: str, reply_to_uri: str = None):
    """Quick function to create a post."""
    reply_to = None
    if reply_to_uri:
        reply_to = await get_reply_context(reply_to_uri)
        if not reply_to:
            console.print(f"[red]Aborting: Could not resolve reply context[/red]")
            return

    async with ComindAgent() as agent:
        return await agent.create_post(text, reply_to=reply_to)
    ...
    if command == "post" and len(sys.argv) > 2:
        ...
        if "--reply-to" in args:
            try:
                ...
                if idx + 1 < len(args):
                    reply_to_uri = args[idx + 1]
                    # Remove flag and value
                    args.pop(idx) # removes flag
                    args.pop(idx) # removes value
                else:
                    console.print("[red]Error: --reply-to requires a URI[/red]")
                    sys.exit(1)
            except ValueError:
                pass
                
        text = " ".join(args)
        asyncio.run(post(text, reply_to_uri=reply_to_uri))
